...
2. After installation run command: ```refreshenv``` or rerun PowerShell (remember that it should be runned as admin)
...
It is posible to install VSCode by choco but default installer is good.
...
3. Scroll upto ```Cmake: Generator``` and enter: ```MinGW Makefiles```
...
4. Input project name, e.g. ```hello_world```  
   ...
   Open ```main.cpp```
5. For building press button ```build``` [(look at the bootom)](/images/install_mingw/vscode/build_run.png)  
6. For running press button ```"Play"``` (around ```Build``` button)
7. For debuggin press ```"Bug"``` (around ```Build``` button)
   [Breakpoint can be placed by clicking aroun line number](/images/install_mingw/vscode/breakpoint.png)  
   For step-by-step debuggin use hot keys: ```F10, F11, F5``` or panel on the top
